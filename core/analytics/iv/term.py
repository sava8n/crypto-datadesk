...
from analytics.frames import empty_frame, expiry_forward, expiry_tte
...
def atm_iv(group: pd.DataFrame, forward: float) -> float | None:
    ...
    sigma = per_strike.to_numpy(dtype=float)
    if strikes.size == 0:
        return None
    return float(np.interp(0.0, np.log(strikes / forward), sigma))
...
def build(prepared_quotes: pd.DataFrame) -> pd.DataFrame:
    ...
    result = (
        pd.DataFrame(rows, columns=TERM_COLUMNS).sort_values("tte_years").reset_index(drop=True)
    )
