...
from shared.quotes import CONTRACT_COLUMNS, prepare_oi_chain, prepare_otm_quotes
...
def make_contracts(forward: float = FORWARD, iv: float = 0.60) -> pd.DataFrame:
    ...
    leg (calls K<F, puts K>=F) and ``prepare_oi_chain`` keeps every row.
    """
    rows = []
    for expiry, tte in _EXPIRIES:
        for i, frac in enumerate(_FRACS):
            strike = forward * frac
            rows.append((expiry, tte, strike, "C", forward, iv, 0.01, 0.009, 0.011, 100.0 + i, 10.0 + i))
            rows.append((expiry, tte, strike, "P", forward, iv, 0.01, 0.009, 0.011, 120.0 + i, 12.0 + i))
